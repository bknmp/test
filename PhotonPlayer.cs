...
public class PhotonPlayer : IComparable<PhotonPlayer>, IComparable<int>, IEquatable<PhotonPlayer>, IEquatable<int>
{
	...
	public bool IsWitness
	{
		get
		{
			object obj = CustomProperties["witness"];
			if (obj == null)
			{
				return false;
			}
	...
	public static PhotonPlayer Find(string userId)
	{
	...
	public override string ToString()
	{
		...
		{
			return string.Format("#{0:00}{1}{2}", ID, IsInactive ? " (inactive)" : " ", IsMasterClient ? "(master)" : "");
		}
		return string.Format("'{0}'{1}{2}", NickName, IsInactive ? " (inactive)" : " ", IsMasterClient ? "(master)" : "");
	}
